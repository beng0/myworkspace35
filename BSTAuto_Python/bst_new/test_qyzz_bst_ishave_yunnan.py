...
if __name__=='__main__':
    ...
    infilename = root_dir + r"\test_sheng_zz_result\云南_省平台qyzz_胡金花_20200728_144206.xlsx"
    outfilename = root_dir + r"\test_sheng_zz_result\云南_省平台qyzz_测试结果_胡金花_"
    ...
    for row in sheet1data:
        sql_zzcode ="""	SELECT zzcode 
                        ...
                        where  name='{qyzz}'  or  match_text='{qyzz}' 
                        or  name like  '%{qyzz}%'  or  match_text like '%{qyzz}%';""".format(schema=myconp[6],qyzz=row[3].strip())
        print(sql_zzcode)
                ...
                if result_zzcode:
                    ...
                    for content in result:
                        if result_zzcode[0][0] == content[2]:
                            ishave_zzcode = "有"
                            break
                    tmp=[row[0],row[1],row[2],row[3],row[4],row[5],row[6],result_zzcode[0][0],ishave_zzcode,ent_key]
    ...
    # 数据保存到Excel   zbr	qyzz	fzjg	fzrq	yxq
    tablenamehouzui = datetime.now().strftime('%Y%m%d_%H%M%S')
    columnRows = ["sheng","shi","zbr", "qyzz", "fzjg", "fzrq","yxq","bst目录表是否有相应的资质","bst相应企业是否有相应的资质","ent_key"]
