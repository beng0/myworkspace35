...
if __name__=='__main__':
    ...
    infilename = root_dir + r"\test_sheng_zz_result\广东_省平台ryzz_胡金花_20200727_173303.xlsx"
    outfilename = root_dir + r"\test_sheng_zz_result\广东_省平台ryzz_测试结果_胡金花_"
    ...
    for row in sheet1data:
        ...
        sql_zzcode ="""SELECT rycode  FROM    "{schema}"."dict_ry_zz"
                            where    match_text='{zzdj}'  
                            and  zclb='{zzdj}'   
                            and   zhuanye='不分专业' ;""".format(schema=myconp[6],zzdj=row[6].strip())
        print(sql_zzcode)
        ...
        # 是否有相应的人员资质
        if result_zzcode:
            ent_key_sql ="""select ent_key from "{schema}"."app_qy_query" WHERE entname = '{zbr}' ;""".format(schema=myconp[5],zbr=row[3].strip())
            ...
            if ent_key_result:
                ...
                sql_string = """with a as ( select unnest(ry_zz_info) as r,ent_key from "{schema}"."app_qy_query" WHERE entname = '{zbr}' 
                                ...
                                from a where r::jsonb->>'name' = '{name}';""".format(schema=myconp[5],zbr=row[3].strip(),name=row[4].strip())
                print(sql_string)
                ...
                for content in result:
                    ...
                    if result_zzcode[0][0] == content[0] or  result_zzcode[0][0] == content[0][0:len(result_zzcode[0][0])]:
                        ishave_zzcode = "有"
                        break
                tmp=[row[0],row[1],row[2],row[3],row[4],row[5],row[6],result_zzcode[0][0],bst_rycode,ishave_zzcode,person_key,ent_key]
            else:tmp=[row[0],row[1],row[2],row[3],row[4],row[5],row[6],result_zzcode[0][0],"",ishave_zzcode,"","bst没有该企业"]
        else:tmp=[row[0],row[1],row[2],row[3],row[4],row[5],row[6],"bst目录表无相应的资质","",ishave_zzcode,"",""]
    ...
    #              href	entname	name	zsbh	zzdj	bst_ryzcode	bst相应人是否有相应的资质
    columnRows = ["sheng","shi","href", "entname", "name", "zsbh","zzdj","省平台code","bst_code","bst相应人是否有相应的资质","person_key","ent_key"]
