...
if __name__=='__main__':
    ...
    infilename = root_dir + r"\test_sheng_zz_result\山西_省平台ryzz_胡金花_20200728_142205.xlsx"
    outfilename = root_dir + r"\test_sheng_zz_result\山西_省平台ryzz_测试结果_胡金花_"
    ...
    for row in sheet1data:
        sql_zzcode = """SELECT rycode  FROM    "{schema}"."dict_ry_zz"  
                                        where    zclb like   '%{zzlb}%' and   zhuanye='不分专业';""".format(schema=myconp[6], zzlb=row[4].strip())
        print(sql_zzcode)
    ...
    # 数据保存到Excel    qyname	name	ryzz	zsbh	zszt
    tablenamehouzui = datetime.now().strftime('%Y%m%d_%H%M%S')
    columnRows = ["sheng", "shi","qyname", "name", "ryzz", "zsbh","zszt","省平台code","bst_code","bst相应人是否有相应的资质","person_key","ent_key"]
