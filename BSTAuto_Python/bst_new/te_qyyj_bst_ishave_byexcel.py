...
for qyyjlis in qyyj_datalist[0][1][1:]:
    ...
    for jstqyyjlis in jst_datalist[0][1][1:]:
        if qyname == jstqyyjlis[2] and qyyj[:5] == jstqyyjlis[3][:5]:
            jsthave = "有"
            break
        else:
            jsthave = "无"
...
for jstqyyjlis in jst_datalist[0][1][1:]:
    ...
    for qyyjlis in qyyj_datalist[0][1][1:]:
        if jstqyname == qyyjlis[4] and jstqyyj[:5] == qyyjlis[2][:5]:
            bsthave = "有"
            break
        else:
            bsthave = "无"
...
outfilename_gd_qyzz = root_dir + r"\get_bst_qyyj\企业业绩对比_贺家斌_"
