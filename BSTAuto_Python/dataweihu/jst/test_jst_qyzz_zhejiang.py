...
class  TestGetJstRyzz(unittest.TestCase):
    ...
    def test_1_get_jst_qyzz_zhejiang(self):
        conp=["zl_reader", "zl_reader", "192.168.60.61:5433", "biaost", "zl_test", "public"]
        total_ye = 6
        yewu = 'qyzz'
        shengfen='zhejiang'
        tablename='jst_qyzz_zhejiang'+self.tablenamehouzui
        outfilename = r"D:\SVN\数据对比\数据准备\企业资质\浙江\浙江建设通企业资质_数据准备_胡金花"
        data = []
        ...
        if int(s) == 1:
            for gotoyema  in  range(1,total_ye):
                self.get_jst_qyzz(jst_qyzz_page, conp, total_ye,gotoyema, shengfen, yewu, tablename,data)
